...
require('./ItsMeHerman.js')
nocache('./ItsMeHerman.js', module => console.log(`${module} telah di update!`))
...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    fs.existsSync('./HermanChanel.json') && zeroyt7.loadAuthInfo('./HermanChanel.json')
    ...
    await zeroyt7.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./HermanChanel.json', JSON.stringify(zeroyt7.base64EncodedAuthInfo(), null, '\t'))

    zeroyt7.on('chat-update', async (message) => {
        require('./ItsMeHerman.js')(zeroyt7, message, _welkom)
    })
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, wel, "©Salam Kenal☕", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Leave To Message", buff, but)
      }
